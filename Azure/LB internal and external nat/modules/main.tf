...
module "registrar_network_security_rule" {
  source = "./network_security_rule"

  for_each = var.registrar_network_security_rule
...
module "registrar_network_interface_security_group_association" {
  source = "./network_interface_security_group_association"
  for_each = var.registrar_linux_virtual_machine
...
module "registrar_network_interface" {
...
module "registrar_linux_virtual_machine" {
...
module "registrar_managed_data_disk" {
  source = "./managed_disk"

  for_each = var.registrar_managed_data_disk
...
module "registrar_virtual_machine_data_disk_attachment" {
  source = "./virtual_machine_data_disk_attachment"

  for_each = var.registrar_managed_data_disks_attachment
...
module "internal_load_balancer_backend_address_pool_association" { # do multiple modules for this if needed
  ...
  backend_address_pool_id             = each.value.internal_backend_address_pool_id
}
module "external_load_balancer_backend_address_pool_association" { # do multiple modules for this if needed
  source = "./load_balancer_backend_address_pool_association"
  depends_on = [module.load_balancer_backend_address_pool]
  for_each = var.registrar_linux_virtual_machine
  ...
  network_interface_id                = each.value.nic_id
  ip_configuration_name               = each.value.ip_configuration_name 
  backend_address_pool_id             = each.value.external_backend_address_pool_id
}
